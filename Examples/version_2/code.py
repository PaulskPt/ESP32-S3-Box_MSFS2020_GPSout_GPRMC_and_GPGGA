...
# Pre-define necessary to prevent error
def clr_touch():
    pass
...
if use_touch:
    ...
    # Attempt to 'clear' the 'touches event buffer'
    ts = clr_touch()
else:
    TX = None
    RX = None
...
max_text_len = 25
#font =  terminalio.FONT  # This is the standard font. It does not have a 'degrees sign'
...
def ck_touch():
    ...
    if tt.touched:
        ...
        print(f"ck_touched(): screen touched at: {ts}")
    else:
        tt_touched = False
    return ts
...
def clr_touch():
    ...
    while tt.touched:
        ...
        if cnt >= 100:
            ret = False
            break
    print(f"clr_touch(): touches count= {tt_cnt}")
    if tt_cnt > 0:
        tt_touched = False


def menu_touch(ts):
    global pge1_group, pge1_lbl, lTouchMsgShown
    TAG= "menu_touch(): "
    touched = False
    t = type(ts)
    if isinstance(ts, list):
        ...
        if isinstance(ts_dict, dict):
            if not lTouchMsgShown:
                # make copy
                tmp = [None, pge1_lbl[1].text, pge1_lbl[2].text, pge1_lbl[3].text, pge1_lbl[4].text, pge1_lbl[5].text]
                touch_y = ts_dict['y']
                touch_x = ts_dict['x']
                touch_id = ts_dict['id']
                touch_pressure = ts_dict['pressure']
                print(TAG+f"Touched: id= {touch_id}, at x,y = {touch_x},{touch_y}. Pressure= {touch_pressure}")
                t_lst = ['id','x','y','pressure',' ']
                le = len(t_lst)
                for _ in range(le):
                    if _ == 0:
                        t1 = touch_id
                        ...
                        t1 = ' '*max_text_len
                    if _ < le-1:
                        pge1_lbl[_+1].text = 'touch {} = {}'.format(t_lst[_],t1)
                    else:
                        pge1_lbl[_+1].text = '{}'.format(t1)
                display.show(pge1_group)
                #while not tt.touched:
                #    time.sleep(0.1)
                time.sleep(2)
                for _ in range(le):
                    pge1_lbl[_+1].text = tmp[_+1]
                pge1_lbl[1].text = tmp[1]
                lTouchMsgShown = True
                touched = True
        else:
...
def loop():
    global startup, start_t, lp_cnt, led_state, biLdIsOn, msg_nr, ctrl_c_flag, diagn_dict, pge1_group, pge1_lbl, scale, font, color, msg_rx_ok, msg_rx_fail, lacTaxyMsgShown, lTouchMsgShown
    TAG = "loop(): "
    if my_debug:
        print(TAG+"...")
            ...
            if chrs_rcvd > 0:
                ...
                if lResult == True:
                    ...
                    if not is_ac_stopped():
                        ...
                        if use_touch and not lTouchMsgShown:
    ...
    while True:
        try:
            ...
            if cnt == 0:
                ...
                lResult = loop()
                cnt += 1
                if lResult == False and ctrl_c_flag == False:
                    # Ctrl+C has been pressed
                    if not my_debug:
                        print(f"loop(): setup returned with: \"{lResult}\"")
                        raise KeyboardInterrupt
        except KeyboardInterrupt:
